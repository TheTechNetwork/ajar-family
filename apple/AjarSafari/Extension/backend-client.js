 ...
 *
 * The block page used to INFER a refusal from a temporary BLOCK rule in the
 * snapshot — and a refusal writes a grant that expires after five minutes,
 * which the backend then drops. So a refused child saw the answer briefly and
 * the page went back to "waiting on a parent" for up to a week. This is the
 * server saying what happened instead.
 *
 * Grants nothing and is consulted by no enforcement path: `decide()` still
 * reads only the signed snapshot. This exists to make a SCREEN honest.
 */
export async function getAnswers() {
  const cfg = await getConfig();
  if (!cfg.backendUrl || !cfg.deviceToken || !cfg.deviceId) throw new Error("not enrolled");
