...
///
/// DUPLICATED from apple/AjarParent/App/Theme.swift and apple/AjarFilter. The
/// three Apple apps are separate Xcode projects with no shared framework between
/// them, so this is a real copy, like the inline CSS copies the extensions
/// carry. If a token changes in tokens.css it must change in FOUR places, which
/// is what `apple/check-theme-tokens.mjs` is for — and that script has to name
/// this file, or the copy nobody checks is the one that drifts.
///
