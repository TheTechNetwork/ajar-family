...
-- Passkeys. Nothing in this table is secret: a public key is public, so a dump
-- of it is not a set of account takeovers.
CREATE TABLE IF NOT EXISTS webauthn_credentials (
  id TEXT PRIMARY KEY, user_id TEXT NOT NULL, public_key_cose TEXT NOT NULL,
  alg INTEGER NOT NULL, sign_count INTEGER NOT NULL DEFAULT 0,
  label TEXT NOT NULL, backed_up INTEGER NOT NULL DEFAULT 0,
  created_at TEXT NOT NULL, last_used_at TEXT
);
CREATE INDEX IF NOT EXISTS idx_webauthn_user ON webauthn_credentials(user_id);
-- Challenges are deleted on redemption, which is what makes them single-use.
CREATE TABLE IF NOT EXISTS webauthn_challenges (
  challenge TEXT PRIMARY KEY, user_id TEXT, kind TEXT NOT NULL,
