...
> **SUPERSEDED — this described the automatic path, and CI no longer uses it.**
> `testflight.yml` once passed `-allowProvisioningUpdates` with the App Store
> Connect API key and let Xcode on the runner mint the distribution certificate
> and every provisioning profile. That stopped working: automatic provisioning
> hits an Apple-side cap on how much it will create, so the workflow now signs
> **manually** (`CODE_SIGN_STYLE=Manual`, `signingStyle: manual`) with a
> certificate and four profiles made **by hand** and handed to it as secrets.
>
> The paragraph is kept rather than deleted because "nothing is generated by
> hand" was left standing here long after §5 said the opposite, and a reader who
> stopped at this section did nothing, created no profile, and met
> `Not configured: APPLE_PROFILE_SAFARI(secret)` at the preflight — which is how
> this contradiction was actually found. **§5 is authoritative for profiles and
> §3.1 for the certificate.**
>
> The API key is still required, for uploading and for build numbers
> (`asc-next-build.mjs`); it just no longer creates anything.
